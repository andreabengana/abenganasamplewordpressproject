    ...
    <div class="section container wow fadeIn" data-wow-duration="1s">
        <div class="row">
            <div class="landing-filter">
                <div class="filter-heading">EVENTS FILTER</div>
                <form action="" method="POST" id="filter">
                    <?php   
                        $categories = get_categories(array('taxonomy' => 'event_type', 'hide_empty' => false ));
                        $topics = get_categories(array('taxonomy' => 'event_topic','hide_empty' => false));
                        
                        $select2 = "<select name='cat' id='topic' class='postform'>n";
                        $select2.= "<option value=''>Topic</option>n";

                        foreach($topics as $topic){
                            $select2.= "<option value='".$topic->slug."'>".$topic->name."</option>";
                        }
                        $select2.= "</select>";
                        ...
                        print '<option value="" selected="selected")>Select date</option>';
        ...
        <div class="row" id="news-list-wrapper">
            ...
            <?php
                if(!empty($type) || !empty($cat)){
                    echo "You are looking for events with the tag(s) <u>".$type. '</u>&nbsp;<u>', $cat,"</u>"; 
                    if(!empty($year)){ echo " posted in the year:", $year; }
                } else {
                    echo "You are looking for events held in the year &nbsp;", $year;
                }
            ?>
            </div>
            <?php
                echo do_shortcode('
                    ...
                    taxonomy="event_type:event_topic" 
